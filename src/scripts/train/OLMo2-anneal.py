...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            f"Usage: torchrun [OPTS..] {sys.argv[0]} [dry_run] model_size run_name checkpoint [OVERRIDES...]"
        )
        sys.exit(1)
    ...
    if dry_run:
        _, model_size, run_name, checkpoint, *overrides = sys.argv[1:]
    else:
        model_size, run_name, checkpoint, *overrides = sys.argv[1:]
        prepare_training_environment()
